...
def blog(request):
    ...
    try:
        ...
        blogs = p.page(1)
    return render(request, 'blog.html' , context={"blogs": blogs, "page_obj":blogs,"recent_blogs":recent_blogs })
...
def blogpage(request, id):
    try:
        ...
        if request.method == 'POST' and not request.user.is_authenticated:
            ...
            Comments.objects.create(blog = blog,anonymous_user=name,person_email=email,  comment=comment)
